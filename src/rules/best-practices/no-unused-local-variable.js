    ...
    for (const stmt of list) {
        ...
        if (stmt.type === 'ApexLocalVariableDeclaration') {
            for (const d of stmt.declarators || []) {
                ...
                if (d.init) {
                    collectExprUsage(d.init, declared, used);
                }
            }
        } else {
...
function collectStmtUsage(stmt, declared, used) {
    ...
    if (stmt.expression) {
        collectExprUsage(stmt.expression, declared, used);
    }
    if (stmt.condition) {
        collectExprUsage(stmt.condition, declared, used);
    }
    if (stmt.argument) {
        collectExprUsage(stmt.argument, declared, used);
    }
    if (stmt.body) {
        collectVarUsage(stmt.body, declared, used);
    }
    if (stmt.block) {
        collectVarUsage(stmt.block, declared, used);
    }
    if (stmt.consequent) {
        collectStmtUsage(stmt.consequent, declared, used);
    }
    if (stmt.alternate) {
        collectStmtUsage(stmt.alternate, declared, used);
    }
    if (stmt.handlers) {
    ...
    if (stmt.finalizer) {
        collectVarUsage(
            stmt.finalizer.block ? stmt.finalizer.block.body || [] : [],
            declared,
            used,
        );
    }
...
function collectExprUsage(expr, declared, used) {
    ...
    if (expr.left) {
        collectExprUsage(expr.left, declared, used);
    }
    if (expr.right) {
        collectExprUsage(expr.right, declared, used);
    }
    if (expr.object) {
        collectExprUsage(expr.object, declared, used);
    }
    if (expr.callee) {
        collectExprUsage(expr.callee, declared, used);
    }
    if (expr.arguments) {
        for (const a of expr.arguments) {
            collectExprUsage(a, declared, used);
        }
    }
    if (expr.argument) {
        collectExprUsage(expr.argument, declared, used);
    }
    if (expr.index) {
        collectExprUsage(expr.index, declared, used);
    }
    if (expr.init) {
        collectExprUsage(expr.init, declared, used);
    }
    if (expr.condition) {
        collectExprUsage(expr.condition, declared, used);
    }
    if (expr.consequent) {
        collectExprUsage(expr.consequent, declared, used);
    }
    if (expr.alternate) {
        collectExprUsage(expr.alternate, declared, used);
    }
    if (expr.expression) {
        collectExprUsage(expr.expression, declared, used);
    }
}
