...
function collectMethodCalls(stmts, calledNames) {
    ...
    for (const stmt of list) {
        ...
        if (stmt.body) collectMethodCalls(stmt.body, calledNames);
        if (stmt.block) collectMethodCalls(stmt.block.body || [], calledNames);
        if (stmt.consequent) collectMethodCalls([stmt.consequent], calledNames);
        if (stmt.alternate) collectMethodCalls([stmt.alternate], calledNames);
        if (stmt.handlers) {
        ...
        if (stmt.declarators) {
            for (const d of stmt.declarators) collectExprCalls(d.init, calledNames);
        }
    }
}
...
function collectExprCalls(expr, calledNames) {
    ...
    if (expr.type === 'ApexMethodCallExpression') {
        ...
        for (const arg of expr.arguments || []) collectExprCalls(arg, calledNames);
        collectExprCalls(expr.callee, calledNames);
    }
    if (expr.left) collectExprCalls(expr.left, calledNames);
    if (expr.right) collectExprCalls(expr.right, calledNames);
    if (expr.object) collectExprCalls(expr.object, calledNames);
    if (expr.argument) collectExprCalls(expr.argument, calledNames);
    if (expr.condition) collectExprCalls(expr.condition, calledNames);
    if (expr.consequent) collectExprCalls(expr.consequent, calledNames);
    if (expr.alternate) collectExprCalls(expr.alternate, calledNames);
    if (expr.init) collectExprCalls(expr.init, calledNames);
    if (expr.index) collectExprCalls(expr.index, calledNames);
}
